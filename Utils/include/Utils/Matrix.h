...
namespace gol
{
	...
	public:
		Matrix() = default;
		Matrix(size_t width, size_t height, std::function<T(size_t, size_t)> creatorFunction);
		~Matrix();
		void forEach(std::function<void(size_t, size_t, T&)> function);
		T& at(size_t x, size_t y);
		T& at(Vector2s position);
	};

	template <typename T>
	Matrix<T>::Matrix(size_t width, size_t height, std::function<T(size_t, size_t)> creatorFunction)
	{
		this->_array = std::make_unique<T[]>(width * height);
		for (size_t x = 0; x < width; x++)
			for (size_t y = 0; y < height; y++)
				this->_array[(x * width) + y] = creatorFunction(x, y);
	...
	template <typename T>
	T& Matrix<T>::at(Vector2s position)
	{
